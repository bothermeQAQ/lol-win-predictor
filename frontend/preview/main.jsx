/* =========================================================================
   PREVIEW APP — orchestration + state. Uses MockAPI; the real Next.js page
   uses the same logic against fetch(NEXT_PUBLIC_API_BASE_URL).
   ========================================================================= */
function App() {
  ...
  const API_BASE = "mock://demo";

  /* ---- boot: meta + ddragon version ---- */
  const boot = useCallback(async () => {
    setError(null); setMeta(null);
    try {
      const [m, v] = await Promise.all([window.MockAPI.meta(), window.LolDDragon.version()]);
      setVersion(v);
      // default draft: first 10 distinct champions, blue 0-4 / red 5-9
  ...
  useEffect(() => {
    ...
    window.MockAPI.predict(body).then((res) => {
      if (cancelled) return;
  ...
  function applyResult(res, cause) {
    ...
    if (big) {
      ...
      // dead-water ripple — a barely-there tick
      setSurge(true);
